...
2. **萃取共同行為**：識別所有元件的共同行為（`GetSize()`）與共同屬性（`Name`、`Parent`），萃取為抽象基底類別 `FileSystemComponent`。
...
- **取捨**：Leaf 繼承了 Component 的所有介面，介面中的部分方法對 Leaf 無意義，可能衍生出操作安全性的問題。
...
| `PasteCommand`  | 移除貼上的副本 | clipboard 已是快照；首次 Execute 時 DeepCopy，Redo 重用同一副本 |
...
- **高建構成本（Expensive Construction）**：物件的初始化過程涉及昂貴的運算或資源存取，從頭建構的成本很高。在本專案中，若被複製的節點是 Directory，必須遞迴走訪並複製其內部所有子節點，同時維護每個節點的 `Parent` 雙向參照。結構越深越廣，從頭建構的成本就越高。
- **狀態保留（State Retention）**：深拷貝產生的副本必須攜帶原物件的完整狀態與結構，而非從空白狀態開始——名稱、標籤、各子型別的專屬屬性（PageCount、Width/Height、Encoding），以及 Directory 內部整棵子樹的組成。只有物件本身才完整知道什麼構成了自己的「完整狀態」，外部程式碼若要逐一設值就必須對每種具體型別的內部有完整的了解。

三條 Force 合在一起，把所有其他建立物件的方式都排除了——不能直接 `new`（Dynamic Creation）、不能從頭建構（Expensive Construction）、不能建立空白物件再逐一設值（State Retention）。唯一剩下的路就是讓物件自己複製自己，因為**只有物件本身同時擁有關於自己具體類別、初始化結果、與完整內部狀態的知識**。

#### Problem

如何對 Composite 樹狀結構中的任意節點進行深拷貝，產生一棵保留完整狀態且與原樹完全獨立的副本？
...
**設計決策——複製時快照**：`copy` 指令立即呼叫 `DeepCopy()` 產生快照存入剪貼簿，確保副本反映的是使用者決定複製那一刻的狀態——copy 與 paste 之間對原件的任何修改不影響貼上結果。`paste` 時再對快照呼叫 `DeepCopy()`，使每次貼上產生獨立副本。Redo 時重用首次產生的副本，避免重複深拷貝且確保名稱一致。
...
使用者輸入 `copy 專案文件 (Project_Docs)`。CLI 在 Composite 結構中找到目標元件，立即呼叫 `DeepCopy()` 產生快照存入剪貼簿。此步驟不建立 Command（非狀態修改操作），但會觸發深拷貝以捕捉複製當下的完整狀態。
...
| 延遲拷貝 vs 即時拷貝 | 即時快照     | `copy` 時立即 `DeepCopy()` 產生快照，確保副本反映複製當下的狀態                  |
